...
Some API's are public API's which are available to clients without login.  These API's may be specified as follows in the configuration file:
    ...
    public-api = api login_api hello_api_api
...
Server configuration note:  If you customize `public-api` configuration, you should set the first two API's to `api` and `login_api`.

For experts:  The reason for recommending `api` and `login_api` to be the first two API's is that many clients expect these to be first two API's.
...
Username/password access is only available through websockets, since login is inherently *stateful*.  We can use the `wscat` program to demonstrate the login functionality:
    ...
    {"jsonrpc": "2.0", "params": ["api", "get_dynamic_global_properties", []], "id":1, "method":"call"}
    < (ok)

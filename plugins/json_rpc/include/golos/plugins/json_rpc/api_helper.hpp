...
// if (n_args > 2) {
//     skip_flags = GOLOS_CONVERT_PARAM(skip_flags,args.args->at(2),uint32_t);
// }
...
#define     PLUGIN_API_ARGS_NUM_ASSERT_I(less, req, all) \
                PLUGIN_API_ARGS_NUM_ASSERT_II(AN_ASSERT_COND(less, req, all), AN_ASSERT_MSG(less, req, all), AN_ASSERT_VARS(less, req, all))
#define     PLUGIN_API_ARGS_NUM_ASSERT_II(cond, msg, vars) \
                GOLOS_ASSERT(cond, golos::invalid_arguments_count, msg, ("n", n_args) vars);
...
#define     AN_ASSERT_VARS(less, req, all) BOOST_PP_IF(less, ("min",req)("max",all), ("required",all))
...
#define     LOAD_ARG(arg, i) ARG_NAME(arg) = GOLOS_CONVERT_PARAM(ARG_NAME(arg), args.args->at(i), ARG_TYPE(arg));

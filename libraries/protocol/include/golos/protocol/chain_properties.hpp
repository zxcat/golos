...
namespace golos {
    namespace protocol {
        ...
        struct chain_properties<uint8_t Major, uint8_t Hardfork, uint16_t Release, type_traits::static_range<Hardfork <= 16>> {
            /**
        ...
        template<uint8_t Major, uint8_t Hardfork, uint16_t Release>
        struct chain_properties<uint8_t Major, uint8_t Hardfork, uint16_t Release,
                type_traits::static_range < Hardfork >= 17>> {
        /**
         *  This fee, paid in STEEM, is converted into VESTING SHARES for the new account. Accounts
         *  without vesting shares cannot earn usage rations and therefore are powerless. This minimum
         *  fee requires all accounts to have some kind of commitment to the network that includes the
        ...
        protocol::reward_curve author_distribution_curve = protocol::reward_curve::quadratic;
        protocol::reward_curve curation_distribution_curve = protocol::reward_curve::quadratic_curation;
...
FC_REFLECT((golos::protocol::chain_properties<0, 16, 0>),
           (account_creation_fee)(maximum_block_size)(sbd_interest_rate));
FC_REFLECT((golos::protocol::chain_properties<0, 17, 0>),
           (account_creation_fee)(maximum_block_size)(sbd_interest_rate)(author_reward_curve)(curation_reward_curve));

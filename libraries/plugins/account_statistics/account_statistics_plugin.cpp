...
namespace steemit {
    namespace plugins {
        namespace account_statistics {
            ...
            void account_statistics_plugin::set_program_options(
                    boost::program_options::options_description &cli,
                    boost::program_options::options_description &cfg
            ) {
                cli.add_options()
                        ("account-stats-bucket-size", boost::program_options::value<string>()->default_value("[60,3600,21600,86400,604800,2592000]"), "Track account statistics by grouping orders into buckets of equal size measured in seconds specified as a JSON array of numbers")
                        ("account-stats-history-per-bucket", boost::program_options::value<uint32_t>()->default_value(100), "How far back in time to track history for each bucker size, measured in the number of buckets (default: 100)")
                        ("account-stats-tracked-accounts", boost::program_options::value<string>()->default_value("[]"), "Which accounts to track the statistics of. Empty list tracks all accounts.");
                cfg.add(cli);
            }
            ...
            void account_statistics_plugin::plugin_initialize(const boost::program_options::variables_map &options) {
                try {
                    ...
                    database().post_apply_operation.connect(
                            [&](const operation_notification &o) { _my->on_operation(o); });

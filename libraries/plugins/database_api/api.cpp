        ...
        namespace database_api {
            ...
            std::vector<extended_account> api::api_impl::get_accounts(std::vector<std::string> names) const {
                ...
                for (auto name: names) {
                    ...
                    if (itr != idx.end()) {
                        ...
                        if (_follow_api) {
                            follow::get_account_reputations_a args;
                            args.account_lower_bound = itr->name;
                            args.limit = 1;
                            results.back().reputation = _follow_api->get_account_reputations_native(
                                    args).reputations[0].reputation;
                        }
            ...
            vector<asset<0, 17, 0>> api::api_impl::get_account_balances(account_name_type acnt, const flat_set<
                ...
                if (assets.empty()) {
                    ...
                    auto range = database().get_index<account_balance_index>().indices().get<
                            by_account_asset>().equal_range(boost::make_tuple(acnt));
                    for (const account_balance_object &balance : boost::make_iterator_range(range.first,
                                                                                            range.second)) {
            ...
            void api::api_impl::set_pending_payout(discussion &d) const {
                ...
                if (database().has_hardfork(STEEMIT_HARDFORK_0_17__91)) {
                    total_r2 = to256(
                            database().get_reward_fund(database().get_comment(d.author, d.permlink)).recent_claims);
                } else {
          ...
          */
                    ...
                    if (_follow_api) {
                        ...
                        d.author_reputation = _follow_api->get_account_reputations_native(
                                args).reputations[0].reputation;
                    }
                }
            ...
            DEFINE_API(api, get_expiring_vesting_delegations) {
                ...
                return my->database().with_read_lock([&]() {
                    ...
                    const auto &exp_idx = my->database().get_index<vesting_delegation_expiration_index,

...
namespace raw_block {
    ...
    class plugin::impl {
    public:
        impl() : db_(appbase::app().get_plugin<plugins::chain::plugin>().db()) {
        }
         // API
        get_raw_block_r get_raw_block(const get_raw_block_a & args);
    ...
    get_raw_block_r plugin::impl::get_raw_block(const get_raw_block_a & args) {
        get_raw_block_r result;
        const auto &db = database();

        // fc::optional<chain::signed_block> block = db.fetch_block_by_number(args.block_num);        
        auto block = db.fetch_block_by_number(args.block_num);        
        if (!block.valid()) {
            return result;
        }
    ...
    DEFINE_API ( plugin, get_raw_block ) {
        auto tmp = args.args->at(0).as<get_raw_block_a>();
        auto &db = my->database();
        return db.with_read_lock([&]() {
            get_raw_block_r result;
            result = my->get_raw_block(tmp);
            return result;
        });
    }

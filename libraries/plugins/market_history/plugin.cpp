        ...
        namespace market_history {
            ...
            public:
                ...
                struct operation_process_fill_order {
                    ...
                    operation_process_fill_order(plugin_impl &mhp, fc::time_point_sec n) : _plugin(mhp),
                            _now(n) {
                    }
            ...
            void plugin::plugin_impl::update_market_histories(
            ...
            void plugin::plugin_initialize(const boost::program_options::variables_map &options) {
                try {
                    ...
                    impl.reset(new plugin_impl());
                    auto &db = impl->database();

                    db.post_apply_operation.connect([&](const operation_notification &o) {
                        impl->update_market_histories(o);
                    });
                    ...
                    if (options.count("history-per-size")) {
                        impl->maximum_history_per_bucket_size = options["history-per-size"].as<uint32_t>();
                    }

                    wlog("bucket-size ${b}", ("b", impl->_tracked_buckets));
                    wlog("history-per-size ${h}", ("h", impl->maximum_history_per_bucket_size));
            ...
            flat_set<uint32_t> plugin::get_tracked_buckets() const {
                return impl->get_tracked_buckets();
            }

            uint32_t plugin::get_max_history_per_bucket() const {
                return impl->get_max_history_per_bucket();
            }
        }
    }

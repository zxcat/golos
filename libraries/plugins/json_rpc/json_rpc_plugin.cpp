...
namespace steemit {
    namespace plugins {
        namespace json_rpc {
            ...
            struct json_rpc_plugin::json_rpc_plugin_impl {
            public:
                using namespace_api_t = map<string, api_description>;
                ...
                api_method & get_api_method(const std::string&api_namespace,const std::string&name_method){
                    auto api_itr = _registered_apis.find(api_namespace);
                    FC_ASSERT(api_itr != _registered_apis.end(), "Could not find API ${api}", ("api", api_namespace));

                    auto method_itr = api_itr->second.find(name_method);
                    FC_ASSERT(method_itr != api_itr->second.end(), "Could not find method ${method}", ("method", name_method));
                    return method_itr->second;
                }

                json_rpc_response rpc(const fc::variant &message);
            private:
                namespace_api_t _registered_apis;
            };


            json_rpc_response json_rpc_plugin::json_rpc_plugin_impl::rpc(const fc::variant &message) {
            ...
            variant json_rpc_plugin::rpc(
                ...
                call = &(_my->get_api_method(api_namespace,method));

    ...
    namespace plugins {
        namespace languages {
            ...
            */
            ...
            public:
                plugin_impl() : _db(appbase::app().get_plugin<chain::plugin>().db()) {
                }
            ...
            plugin::plugin_impl::~plugin_impl() {
                return;
            }
            ...
            namespace detail {
                struct operation_visitor {
                    ...
                    void remove_tag(const language_object &tag) const {
                        ...
                        const auto &idx = _db.get_index<author_language_stats_index>().indices().get<
                                by_author_tag_posts>();
                        ...
                        if (op.to == STEEMIT_NULL_ACCOUNT && op.amount.symbol_name() == SBD_SYMBOL_NAME) {
                            ...
                            if (part[0].size() && part[0][0] == '@') {
                                ...
                                if (c && c->parent_author.size() == 0) {
                                    const auto &comment_idx = _db.get_index<language_index>().indices().get<
                                            by_comment>();

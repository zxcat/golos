...
public:
   ...
   virtual bool has_item( const network::item_id& ) override;
   virtual bool handle_block( const network::block_message&, bool, std::vector<fc::uint160_t>& ) override;
   virtual void handle_transaction( const network::trx_message& ) override;
   virtual void handle_message( const network::message& ) override;
   virtual std::vector< network::item_hash_t > get_block_ids( const std::vector< network::item_hash_t >&, uint32_t&, uint32_t ) override;
   virtual network::message get_item( const network::item_id& ) override;
   virtual std::vector< network::item_hash_t > get_blockchain_synopsis( const network::item_hash_t&, uint32_t ) override;
   virtual void sync_status( uint32_t, uint32_t ) override;
   virtual void connection_count_changed( uint32_t ) override;
   virtual uint32_t get_block_number( const network::item_hash_t& ) override;
   virtual fc::time_point_sec get_block_time(const network::item_hash_t& ) override;
   virtual fc::time_point_sec get_blockchain_now() override;
   virtual network::item_hash_t get_head_block_id() const override;
   ...
   std::unique_ptr<network::node> node;
...
bool p2p_plugin_impl::has_item( const network::item_id& id )
{
   try
   {
...
bool p2p_plugin_impl::handle_block( const network::block_message& blk_msg, bool sync_mode, std::vector<fc::uint160_t>& )
{ try {
...
void p2p_plugin_impl::handle_transaction( const network::trx_message& trx_msg )
{
   try
   {
...
void p2p_plugin_impl::handle_message( const network::message& message_to_process )
{
   // not a transaction, not a block
   FC_THROW( "Invalid Message Type" );
}

std::vector< network::item_hash_t > p2p_plugin_impl::get_block_ids( const std::vector< network::item_hash_t >& blockchain_synopsis, uint32_t& remaining_item_count, uint32_t limit )
{ try {
...
std::vector<network::item_hash_t > p2p_plugin_impl::get_blockchain_synopsis(
        const network::item_hash_t& reference_point,

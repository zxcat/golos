...
using steemit::network::item_hash_t;
using steemit::network::item_id;
using steemit::network::message;
using steemit::network::block_message;
using steemit::network::trx_message;
...
public:
   ...
   virtual std::vector<item_hash_t > get_block_ids( const std::vector< item_hash_t >&, uint32_t&, uint32_t ) override;
...
std::vector< item_hash_t > p2p_plugin_impl::get_block_ids( const std::vector< item_hash_t >& blockchain_synopsis, uint32_t& remaining_item_count, uint32_t limit ) { try {
   return chain.db().with_read_lock( [&]() {
      ...
      if( blockchain_synopsis.empty() || ( blockchain_synopsis.size() == 1 && blockchain_synopsis[0] == block_id_type() ) ) {
         ...
         for( const item_hash_t& block_id_in_synopsis : boost::adaptors::reverse(blockchain_synopsis) ) {
            if (block_id_in_synopsis == block_id_type() || (chain.db().is_known_block(block_id_in_synopsis) && is_included_block(block_id_in_synopsis))) {
...
std::vector< item_hash_t > p2p_plugin_impl::get_blockchain_synopsis( const item_hash_t& reference_point, uint32_t number_of_blocks_after_reference_point ) {
   try {
...
bool p2p_plugin_impl::is_included_block(const block_id_type& block_id) { try {
   return chain.db().with_read_lock( [&]() {
...
void p2p_plugin::plugin_startup() {
   my->p2p_thread.async( [this] {
      my->node.reset(new steemit::network::node(my->user_agent));
      ...
      my->node->sync_from(item_id(steemit::network::block_message_type, block_id), std::vector<uint32_t>());

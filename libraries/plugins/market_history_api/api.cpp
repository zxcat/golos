...
namespace golos {
    namespace plugins {
        namespace market_history {
            class api::api_impl {
            public:
                api_impl() : _chain(appbase::app().get_plugin<chain::plugin>()) {
                }
            ...
            namespace detail {
                struct operation_process_fill_order_visitor {
                    vector<optional<golos::chain::asset_object>> &assets;

                    operation_process_fill_order_visitor(vector<optional<golos::chain::asset_object>> &input_assets)
                            : assets(input_assets) {
                    }

                    typedef market_trade result_type;

                    double price_to_real(const protocol::share_type a, int p) const {
                ...
                template<> market_trade detail::operation_process_fill_order_visitor::operator()
                <0, 16, 0>(
                const fill_order_operation<0, 16, 0> &o
                ) const {
                market_trade trade;

                if (assets[0]->asset_name == o.open_pays.

                symbol_name()

                ) {
                trade.
                amount = price_to_real(o.current_pays.amount, assets[1]->precision);
                trade.
                value = price_to_real(o.open_pays.amount, assets[0]->precision);
            }
            else {
            trade.
            amount = price_to_real(o.open_pays.amount, assets[1]->precision);
            trade.
            value = price_to_real(o.current_pays.amount, assets[0]->precision);
        }
    ...
    template<> market_trade detail::operation_process_fill_order_visitor::operator()
    <0, 17, 0>(
    const fill_order_operation<0, 17, 0> &o
    ) const {
    market_trade trade;
...
template<> market_trade detail::operation_process_fill_order_visitor::operator()
<0, 16, 0>(
const fill_call_order_operation<0, 16, 0> &o
) const{
market_trade trade;

if (assets[0]->asset_name == o.receives.

symbol_name()

) {
trade.
amount = price_to_real(o.pays.amount, assets[1]->precision);
trade.
value = price_to_real(o.receives.amount, assets[0]->precision);
} else {
trade.
amount = price_to_real(o.receives.amount, assets[1]->precision);
trade.
value = price_to_real(o.pays.amount, assets[0]->precision);
}
...
template<> market_trade detail::operation_process_fill_order_visitor::operator()
<0, 17, 0>(
const fill_call_order_operation<0, 17, 0> &o
) const {
market_trade trade;
...
template<> market_trade detail::operation_process_fill_order_visitor::operator()
<0, 16, 0>(
const fill_settlement_order_operation<0, 16, 0> &o
) const {
...
template<> market_trade detail::operation_process_fill_order_visitor::operator()
<0, 17, 0>(
const fill_settlement_order_operation<0, 17, 0> &o
) const {
market_trade trade;
...
void api::set_subscribe_callback(std::function<void( const variant &
...
void api::api_impl::set_subscribe_callback(std::function<void( const variant &
...
param.
false_positive_probability = 1.0 / 10000;
param.
maximum_size = 1024 * 8 * 8 * 2;
param.
...
void api::set_pending_transaction_callback(std::function<void( const variant &
...
void api::api_impl::set_pending_transaction_callback(std::function<void( const variant &

)> cb) {
_pending_trx_callback = cb;
}

void api::api_impl::set_block_applied_callback(std::function<void( const variant &
...
_block_applied_connection = connect_signal(database().applied_block, *this, &api::api_impl::on_applied_block);
}
...
void api::api_impl::unsubscribe_from_market(string a, string b) {
    if (a > b) {
        std::swap(a, b);
    }
    FC_ASSERT(a != b);
...
vector <optional<asset_object>> api::api_impl::lookup_asset_symbols(
    ...
    std::transform(asset_symbols.begin(), asset_symbols.end(), std::back_inserter(result), [this, &assets_by_symbol](
...
market_ticker api::api_impl::get_ticker(const string &base, const string &quote) const {
    ...
    try {
        ...
        const fc::time_point_sec yesterday = app.chain_database()->fetch_block_by_number(1)->timestamp <
                                             fc::time_point_sec(now.sec_since_epoch() - 86400) ? fc::time_point_sec(
                now.sec_since_epoch() - 86400) : app.chain_database()->fetch_block_by_number(1)->timestamp;
...
market_volume api::api_impl::get_volume(const string &base, const string &quote) const {
...
order_book api::api_impl::get_order_book(const string &base, const string &quote, unsigned limit) const {
    FC_ASSERT(limit <= 50);
    ...
    std::function<double( const price<0, 17, 0>
    &)> price_to_real = [&](const price<0, 17, 0> &p) -> double {
    ...
    for (const auto &o : orders) {
        if (o.sell_price.base.symbol == base_id) {
            ...
            ord.quote = asset_to_real(
                    share_type((o.for_sale.value * o.sell_price.quote.amount.value) / o.sell_price.base.amount.value),
                    assets[1]->precision);
...
std::vector<market_trade> api::api_impl::get_trade_history(const string &base, const string &quote,
                                                           fc::time_point_sec start, fc::time_point_sec stop,
...
vector <order_history_object> api::api_impl::get_fill_order_history(const string &a, const string &b,
                                                                    uint32_t limit) const {
    ...
    const auto &history_idx = db.get_index<golos::market_history::order_history_index>().indices().get<by_key>();
    history_key hkey;
    hkey.base = a_name;
    hkey.quote = b_name;
    hkey.sequence = std::numeric_limits<int64_t>::min();
...
vector <bucket_object> api::api_impl::get_market_history(const std::string &a, const std::string &b,
    ...
    try {
        ...
        while (itr != by_key_idx.end() && itr->key.open <= end && result.size() < 200) {
            if (!(itr->key.base == a_name && itr->key.quote == b_name && itr->key.seconds == bucket_seconds)) {
                return result;
            }
            result.emplace_back(*itr);
            ++itr;
...
flat_set <uint32_t> api::api_impl::get_market_history_buckets() const {
 ...
 */
vector <limit_order_object> api::api_impl::get_limit_orders(const string &a, const string &b, uint32_t limit) const {
...
vector <call_order_object> api::api_impl::get_call_orders(const string &a, uint32_t limit) const {
    ...
    price<0, 17, 0> index_price = price<0, 17, 0>::min(
            app.chain_database()->get_asset_bitasset_data(mia.asset_name).options.short_backing_asset, mia.asset_name);

    return vector<call_order_object>(call_index.lower_bound(index_price.min()),
                                     call_index.lower_bound(index_price.max()));
}

vector <force_settlement_object> api::api_impl::get_settle_orders(const string &a, uint32_t limit) const {
...
vector <call_order_object> api::api_impl::get_margin_positions(const account_name_type &name) const {
    try {
...
vector <collateral_bid_object> api::api_impl::get_collateral_bids(const string &asset, uint32_t limit,
                                                                  uint32_t start, uint32_t skip) const {
    try {
        FC_ASSERT(limit <= 100);
...
std::vector<liquidity_balance> api::api_impl::get_liquidity_queue(const string &start_account, uint32_t limit) const {
    ...
    if (start_account.length()) {
        const auto &liq_by_acc = app.chain_database()->get_index<liquidity_reward_balance_index>().indices().get<
                by_owner>();
...
std::vector<golos::application::extended_limit_order> api::api_impl::get_limit_orders_by_owner(
        const string &owner) const {
    ...
    while (itr != idx.end() && itr->seller == owner) {
        ...
        if (itr->sell_price.base.symbol == STEEM_SYMBOL_NAME) {
            result.back().real_price = price_to_real((~result.back().sell_price).base.amount, assets[0]->precision) /
...
std::vector<call_order_object> api::api_impl::get_call_orders_by_owner(const string &owner) const {
...
std::vector<force_settlement_object> api::api_impl::get_settle_orders_by_owner(const string &owner) const {
...
DEFINE_API(api, get_ticker) {
    ...
    return my->database().with_read_lock([&]() {
        return my->get_ticker(args.args->at(0).as<std::string>(), args.args->at(1).as<std::string>());
    });
}
...
DEFINE_API(api, get_volume) {
    CHECK_ARG_SIZE(2)
    return my->database().with_read_lock([&]() {
        return my->get_volume(args.args->at(0).as<std::string>(), args.args->at(1).as<std::string>());
    });
}
...
DEFINE_API(api, get_order_book) {
    CHECK_ARG_SIZE(3)
    return my->database().with_read_lock([&]() {
        return my->get_order_book(args.args->at(0).as<std::string>(), args.args->at(1).as<std::string>(),
...
DEFINE_API(api, get_trade_history) {
    CHECK_ARG_SIZE(5)
    return my->database().with_read_lock([&]() {
        return my->get_trade_history(args.args->at(0).as<std::string>(), args.args->at(1).as<std::string>(),
                              args.args->at(2).as<fc::time_point_sec>(), args.args->at(3).as<fc::time_point_sec>(),
...
DEFINE_API(api, get_fill_order_history) {
    CHECK_ARG_SIZE(3)
    return my->database().with_read_lock([&]() {
        return my->get_fill_order_history(args.args->at(0).as<std::string>(), args.args->at(1).as<std::string>(),
                              args.args->at(2).as<uint32_t>());
    });
}
...
DEFINE_API(api, get_market_history) {
    CHECK_ARG_SIZE(5)
    return my->database().with_read_lock([&]() {
        return my->get_market_history(args.args->at(0).as<std::string>(), args.args->at(1).as<std::string>(),
                              args.args->at(2).as<uint32_t>(), args.args->at(3).as<fc::time_point_sec>(),
...
DEFINE_API(api, get_limit_orders) {
    CHECK_ARG_SIZE(3)
    return my->database().with_read_lock([&]() {
        return my->get_limit_orders(args.args->at(0).as<std::string>(), args.args->at(1).as<std::string>(),
                                              args.args->at(2).as<uint32_t>());
    });
}
...
DEFINE_API(api, get_call_orders) {
    CHECK_ARG_SIZE(2)
    return my->database().with_read_lock([&]() {
        return my->get_call_orders(args.args->at(0).as<std::string>(), args.args->at(1).as<uint32_t>());
    });
}
...
DEFINE_API(api, get_settle_orders) {
    CHECK_ARG_SIZE(2)
    return my->database().with_read_lock([&]() {
        return my->get_settle_orders(args.args->at(0).as<std::string>(), args.args->at(1).as<uint32_t>());
    });
}
...
DEFINE_API(api, get_collateral_bids) {
    CHECK_ARG_SIZE(3)
    return my->database().with_read_lock([&]() {
        return my->get_collateral_bids(args.args->at(0).as<std::string>(), args.args->at(1).as<uint32_t>(), args.args->at(2).as<uint32_t>());
    });
}
...
DEFINE_API(api, get_margin_positions) {
    CHECK_ARG_SIZE(1)
    return my->database().with_read_lock([&]() {
        return my->get_margin_positions(args.args->at(0).as<std::string>());
    });
}
...
DEFINE_API(api, get_liquidity_queue) {
    CHECK_ARG_SIZE(2)
    return my->database().with_read_lock([&]() {
        return my->get_liquidity_queue(args.args->at(0).as<std::string>(), args.args->at(0).as<uint32_t>());
    });
}

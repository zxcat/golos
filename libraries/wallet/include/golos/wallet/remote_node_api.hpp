...
struct remote_private_message {
    vector <message_api_object> get_inbox(const std::string& to, time_point newest, uint16_t limit, std::uint32_t offset) const;
    vector <message_api_object> get_outbox(const std::string& from, time_point newest, uint16_t limit, std::uint32_t offset) const;
};

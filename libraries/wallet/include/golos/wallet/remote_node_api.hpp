...
// #include <golos/plugins/tags_api/tags_api.hpp>
// #include <golos/plugins/follow/follow.hpp>
// #include <golos/plugins/market_history_api/market_history_api.hpp>
// #include <golos/plugins/witness_api/witness_api.hpp>

// #include <golos/plugins/account_history_api/account_history_api_plugin.hpp>
namespace golos {
    namespace wallet {
        ...
        struct remote_node_api {
            ...
            // vector< tags::tag_api_object > get_trending_tags( string, uint32_t );
            ...
            // optional< api::signed_block_api_object > get_block( uint32_t );
            // vector< account_history::operation_api_object > get_ops_in_block( uint32_t, bool only_virtual = true );
            // fc::variant_object get_config();
            api::dynamic_global_property_api_object get_dynamic_global_properties();

            chain_properties get_chain_properties();

            price get_current_median_history_price();

            api::feed_history_api_object get_feed_history();

            api::witness_schedule_api_object get_witness_schedule();

            hardfork_version get_hardfork_version();

            // // condenser_api::scheduled_hardfork get_next_scheduled_hardfork();
            api::reward_fund_api_object get_reward_fund(string);

            std::vector<std::vector<golos::protocol::account_name_type> > get_key_references(vector<public_key_type>);

            std::vector<golos::plugins::database_api::extended_account> get_accounts(vector<account_name_type>);

            // vector< account_id_type > get_account_references( account_id_type account_id );
            // vector< optional< api::account_api_object > > lookup_account_names( vector< account_name_type > );
            vector<account_name_type> lookup_accounts(account_name_type, uint32_t);

            // uint64_t get_account_count();
            vector<api::owner_authority_history_api_object> get_owner_history(account_name_type);

            // optional< api::account_recovery_request_api_object > get_recovery_request( account_name_type );
            // optional< api::escrow_api_object > get_escrow( account_name_type, uint32_t );
            // vector< api::withdraw_vesting_route_api_object > get_withdraw_routes( account_name_type, condenser_api::withdraw_route_type );
            // optional< witness::account_bandwidth_api_object > get_account_bandwidth( account_name_type, witness::bandwidth_type );
            // vector< api::savings_withdraw_api_object > get_savings_withdraw_from( account_name_type );
            // vector< api::savings_withdraw_api_object > get_savings_withdraw_to( account_name_type );
            // vector< api::vesting_delegation_api_object > get_vesting_delegations( account_name_type, account_name_type, uint32_t );
            // vector< api::vesting_delegation_expiration_api_object > get_expiring_vesting_delegations( account_name_type, time_point_sec, uint32_t );
            // vector< optional< api::witness_api_object > > get_witnesses( vector< witness_id_type > );
            vector<api::convert_request_api_object> get_conversion_requests(account_name_type);

            optional<api::witness_api_object> get_witness_by_account(account_name_type);

            // vector< api::witness_api_object > get_witnesses_by_vote( account_name_type, uint32_t );
            ...
            // map< uint32_t, account_history::operation_api_object > get_account_history( account_name_type, uint64_t, uint32_t );
            // void broadcast_transaction( signed_transaction );
            golos::plugins::network_broadcast_api::broadcast_transaction_synchronous_return broadcast_transaction_synchronous(
                    signed_transaction);

            // void broadcast_block( signed_block );
            // vector< follow::follow_api_object > get_followers( account_name_type, account_name_type, follow::follow_type, uint32_t );
            // vector< follow::follow_api_object > get_following( account_name_type, account_name_type, follow::follow_type, uint32_t );
...
FC_REFLECT ((golos::wallet::remote_node_api), (a))
// FC_REFLECT_TYPENAME ((golos::wallet::remote_node_api) )

FC_API(golos::wallet::remote_node_api,
       (get_key_references)(get_dynamic_global_properties)(get_witness_schedule)(get_hardfork_version)(
               get_current_median_history_price)(get_chain_properties)(get_reward_fund)
               // (get_version)
               (get_accounts)(broadcast_transaction_synchronous)(get_witness_by_account)(get_block)(lookup_accounts)(
               get_active_witnesses)(get_feed_history)(lookup_witness_accounts)(get_owner_history)(
